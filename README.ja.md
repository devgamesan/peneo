...
Peneo は、シンプルな TUI（ターミナルユーザーインターフェース）ファイルマネージャです。親ディレクトリ・現在ディレクトリ・子ディレクトリを並べた 3 ペイン構成で、GUI ファイラーに近い直感的な操作感を実現しています。Peneo は、ショートカットキーを極力覚えなくても操作できるよう設計されており、よく使う操作は画面下部のヘルプバーに常時表示されます。その他の操作も、コマンドパレットから直感的に起動できます。


## 特徴

- 親 / 現在 / 子ディレクトリを並べたシンプルな 3 ペイン表示です。ディレクトリ移動、複数選択、コピー、カット、貼り付け、ゴミ箱への移動、パスのコピー、リネーム、新規作成、アーカイブの展開、zip 圧縮、ファイル検索、grep 検索、1 行シェルコマンド実行をキーボードだけで操作できます。よく使う操作は画面下部のヘルプバーに常時表示しています。
...
- 3 ペインの下に埋め込みターミナルを開けます。`Ctrl+T` でブラウザとターミナルをすばやく切り替えられます。ターミナルはカレントディレクトリで開かれるため、ディレクトリ移動の必要がなく、ブラウザでの閲覧とターミナルでの操作をシームレスに行き来できます。
...
- カレントディレクトリ配下を対象とした再帰 grep 検索が可能です。検索結果から該当ファイルへジャンプできます。
...
- grep 検索 (`Ctrl+G`) を使う場合: `ripgrep` (`rg`)
- パスコピーを使う場合:
  - X11 環境: `xclip`
  - Wayland 環境: `wl-copy`
...
| `logging` | `path` | パス文字列 | 任意のログファイル保存先です。空文字なら `config.toml` と同じディレクトリの `peneo.log` を使います。 |
| `bookmarks` | `paths` | 絶対パス文字列の配列 | `Ctrl+B` やコマンドパレットの `Show bookmarks` で使うブックマーク一覧です。重複パスは読み込み時に取り除かれます。 |
...
| 通常時 | `Alt+←` | 履歴を一つ戻る |
| 通常時 | `Alt+→` | 履歴を一つ進む |
| 通常時 | `Ctrl+J` | 特定のパスへ移動するための入力を開き、ディレクトリ補完を使えるようにする |
| 通常時 | `Alt+Home` | ホームディレクトリへ移動 |
| 通常時 | `Ctrl+O` | ディレクトリ履歴リストを開き、選択したディレクトリへ移動 |
| 通常時 | `Ctrl+B` | ブックマークリストを開き、選択したディレクトリへ移動 |
...
| 通常時 | `Ctrl+A` | 現在ディレクトリで表示中の項目をすべて選択 |
...
| 通常時 | `Ctrl+N` | 現在ディレクトリで新規ファイル作成を開始 |
| 通常時 | `Ctrl+D` | 現在ディレクトリで新規ディレクトリ作成を開始 |
...
| `Open terminal` | 常に表示 | `config.toml` の設定を優先しつつ、現在ディレクトリ起点で外部ターミナルを起動します。 |
