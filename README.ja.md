...
| Windows | サポート | ドライブ移動、ファイル操作、クリップボード、シェルコマンド、外部ターミナルなどほとんどの機能を利用できます。`zivo-cd` とゴミ箱復元（undo）は Windows では未対応です。 |
...
| --- | --- | --- | --- | --- |
| ドキュメント preview (`docx` / `xlsx` / `pptx`) | `pandoc` 3.8.3+ | `pandoc` 3.8.3+ | `pandoc` 3.8.3+ | `pandoc` 3.8.3+ |
| 画像 preview (`png` / `jpg` / `gif` / `webp` など) | `chafa` | `chafa` | `chafa` | `chafa` |
...
| `Grep in selected files` | カレントディレクトリでファイルが1つフォーカスされているか、1つ以上のファイルが選択されているとき | 選択されたファイル、または何も選択されていない場合はフォーカスされたファイルに限定してgrep検索を開きます。キーワードを入力してgrep検索を行い、一致した行がパレットに表示されます。`↑` / `↓` または `Ctrl+n` / `Ctrl+p` で結果間を移動し、`Enter` でファイルへナビゲートし、`Ctrl+e` でターミナルエディタでファイルを開きます。 |
...
| `display` | `enable_image_preview` | `true` / `false` | `chafa` を使った画像 preview を右ペインで表示します。既定値は `true` です。`chafa` が未導入の場合は失敗ではなく依存不足メッセージを表示します。 |
