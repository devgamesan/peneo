...
- 親 / 現在 / 右ペインを並べたシンプルな 3 ペイン表示です。カーソルがディレクトリ上にあるときは右ペインに子要素一覧を表示し、一般的なテキストファイル上にあるときは右ペインに構文色分け付きのテキストプレビューを表示します。左右にディレクトリを並べてコピー・移動できる 2 ペイン転送レイアウトにも切り替えられます。ディレクトリ移動、複数選択、コピー、カット、貼り付け、直前の可逆ファイル操作の Undo、ゴミ箱への移動、ファイル削除、パスのコピー、リネーム、新規作成、アーカイブの展開、zip 圧縮、選択ファイル群に対する文字列置換プレビュー、ファイル検索、grep 検索、1 行シェルコマンド実行をキーボードだけで操作できます。よく使う操作は画面下部のヘルプバーに常時表示しています。
...
| `Esc` | フォーカス中ペインの選択を解除 |
| `y` | フォーカス中ペインの対象を反対側ペインへコピー |
| `m` | フォーカス中ペインの対象を反対側ペインへ移動 |
