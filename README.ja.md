...
> **プラットフォームに関する注意**: zivo は現在開発中 (Beta) です。Linux、macOS、Windows で動作しますが、一部の機能にはプラットフォーム固有の制限があります。シェル統合 (`zivo-cd`) およびスタンドアロンネイティブ実行ファイルは Windows では利用できません。OS 別の詳細は [Platforms](docs/platforms.ja.md) を参照してください。
...
3 ペインでディレクトリを移動しながら右ペインでファイルをプレビューできます。ファイル検索・grep機能により、目的のファイルに素早くジャンプできます。よく使う機能はヘルプバーに表示しているため、迷わず操作できます。
...
Transfer モードでは、2つのディレクトリを左右に並べて、選択したファイルを反対側のペインへコピーまたは移動できます。`y` でコピー、`m` で移動でき、転送結果をその場で確認できます。
...
> **注**: 一部の機能（プレビュー、grep、置換）は外部ツールに依存しており、システムにプリインストールされていない場合があります。要件は [インストール](#インストール) を参照してください。プラットフォーム固有の制限については [Platforms](docs/platforms.ja.md) を参照してください。
