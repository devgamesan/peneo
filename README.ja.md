...
- 親 / 現在 / 子ディレクトリを並べたシンプルな 3 ペイン表示です。ディレクトリ移動、複数選択、コピー、カット、貼り付け、削除(ゴミ箱への移動)、パスのコピー、リネーム、新規作成、アーカイブの展開、アーカイブ作成、ファイル検索、grepをキーボードだけで操作できます。よく使う操作は画面下部のヘルプバーに常時表示しています。
...
- カレントディレクトリ配下の全ファイルを対象とした全文検索（grep）が可能です。検索結果から該当するファイルにキー操作ひとつでダイレクトにジャンプできます。
...
- ファイルやディレクトリは OS の既定アプリで開けます。例えば、カレントディレクトリをOSのファイルマネージャでオープンしたり、ファイルをVSCodeで開くといったことが可能です。また、規定のターミナルを別ウィンドウで起動することもできます。
