...
| `Replace text in grep results` | Always | Opens a five-field replacement palette (keyword, include, exclude, find, replace). Type a keyword to grep search, optionally filter by include/exclude extensions, then type find/replace text to preview replacements across matched files. `Tab` / `Shift+Tab` cycle between fields. The right pane shows the diff preview, and `Enter` applies the replacement. |
