...
| `Enter` | 選択中の結果を開く |
| `Ctrl+w` | ファイル検索結果を新規タブの Search Workspace として開く |
...
| `/` | フィルタモードに入る |
| `s` | 並び順を切り替える |
| `m` | grep 結果の match/file 表示を切り替える |
| `Enter` | 選択中の結果を開く |
| `C` | 選択中のパス、または未選択時はカーソル位置のパスをシステムクリップボードへコピー |
| `:` | コマンドパレットを開く |
| `Tab` / `Shift+Tab` | タブ切り替え |
| `w` | Search Workspace タブを閉じる |
| `Esc` | 選択を解除 |

Search Workspace はファイル検索結果を新規タブに開く仮想一覧です。元のブラウズタブを残したまま、検索結果を作業対象リストとして確認できます。
